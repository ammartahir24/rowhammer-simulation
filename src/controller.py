	...
	def row_activate_cb(self, bank, row):
		...
		if self.configs.para:
			if random.randrange(1000) < 2:
				options = [(row-i-1, bank) for i in range(self.configs.para_ref_rows) if row-i-1>=0]
				options += [(row+i+1, bank) for i in range(self.configs.para_ref_rows) if row+i+1<self.configs.rows]
