...
def p1_read(commandseq, value):
	global clock, log
	log.write("%d read callback %s %s time taken (ns): %d" %(clock.get_clock(), hex(commandseq.address), bin(value), commandseq.completion_time - commandseq.arrival_time))
...
for row_addr in r_addrs:
	...
	for column in range(2**(cfg.col_bits)):
		...
		program1.cmd(program1.read, (addr, p1_read), 50+t*50, period = 9000, repeat=1000)
		t+=1
		addr = addr + 1

clock.schedule(print_row_refreshes, None, run_time=9999999)
